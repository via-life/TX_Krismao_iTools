...
需求一和需求三共用本地安装的 iTools Chrome 助手 2.1.3：

- 需求一由扩展后台访问固定测试/正式上传接口及广州内网 COS；网页不能传入任意接口或主机。
- 需求三先以无 Cookie 模式读取固定元宝下载端点，避免通配 CORS 与凭据冲突；401/403 时再由扩展使用当前 Chrome 登录态回退，并把图片转为本地 Blob 后生成 PNG。
- 扩展不申请 `cookies` 权限，不读取、显示或保存 Cookie。
- 插件源码、私有配置、安装 ZIP、构建脚本和 `启动.bat` 回退包只存放在本地交付目录，不在本仓库提供下载。

首次使用时，从本地交付目录选择 `iTools浏览器助手-v2.1.3`：打开 `chrome://extensions/`，开启开发者模式，点击“加载已解压的扩展程序”，然后刷新需求一或需求三页面。
...
- 需求三必须使用已登录且有图片权限的 Chrome；过期或无权限图片只能降级为原链接。
- Chrome 不允许普通网页静默安装本地扩展，首次安装、升级和凭据轮换后均需在扩展页手动重新加载。
