...
生成内嵌 PNG Excel 时会以最多 4 路并发读取图片，并在截图当前会话时预取下一会话；相同图片复用受 24 项/96 MiB 上限保护的缓存。默认“高清模式”会把完整 PNG 嵌入工作簿，Excel 中仅按合法行高显示缩略图，放大或另存图片时仍保留原始清晰度；“省内存快速模式”才会按 900 × 546 px 的实际显示上限直接渲染。两种模式的行高都不超过 Excel 允许的 409.5 磅。PNG 使用 Blob 保存且画布编码后立即释放，避免二进制副本持续占用内存；高清模式同时只保留一个待编码画布。Excel 打包使用快速压缩级别，完成提示会分别显示 PNG、Excel 打包和总耗时。
