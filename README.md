...
生成内嵌 PNG Excel 时会以最多 4 路并发读取图片，并在截图当前会话时预取下一会话；相同图片复用受 24 项/96 MiB 上限保护的缓存。Excel 专用截图按工作簿实际展示上限 900 × 1400 px 直接渲染，PNG 使用 Blob 保存且画布编码后立即释放，避免超长会话的大画布和二进制副本持续占用内存。PNG 编码与下一会话截图流水执行，Excel 打包使用快速压缩级别，完成提示会分别显示 PNG、Excel 打包和总耗时。
