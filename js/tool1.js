...
(function () {
  ...
  function applyEnv() {
    ...
    if (helperEnvReady(mode)) {
      ...
      setStatus(el['env-hint'], 'Chrome 助手已连接，但当前安装包未配置需求一上传。请安装“本地插件交付”目录中的 2.1.6 私有扩展，或使用其中的“需求一本地回退”。', 'warn');
      ...
      setStatus(el['env-hint'], 'Chrome 助手与本地上传服务均不可用。请确认 2.1.6 私有扩展已启用，或使用本地交付中的“需求一本地回退”。', 'error');
  ...
  function updateUploadHelperStatus(status, version) {
    ...
    if (uploadHelper.connected) {
      ...
      el['upload-helper-status'].textContent = '未检测到可用于需求一的 Chrome 上传助手，请安装“本地插件交付”目录中的 2.1.6 私有扩展。';
    }
    ...
    if (uploadHelper.connected) {
      ...
      el['upload-helper-capabilities'].textContent = ready ?
        ...
        '当前安装包未包含需求一配置；请使用本地插件交付中的 2.1.6 私有扩展。';
    }
    applyEnv();
  }
